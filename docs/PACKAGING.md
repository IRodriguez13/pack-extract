...
| Binarios C (no bash en `/usr/bin`) | Pendiente local | `sudo apt install libarchive-dev && sudo ./install.sh` |
...
| CI (GitHub Actions) | Falta | build + `make check` en ubuntu-latest |
...
3. **`dist-pack` tarball:** no incluye `COPYING` ni `debian/` — ampliar `PACK_FILES` si el release upstream debe empaquetar Debian.
